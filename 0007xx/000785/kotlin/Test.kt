...
fun test(graphStr: String, expected: Boolean) {
  println("graph=%s, expected=%s".format(graphStr, expected))
  val graph0 = toIntQAryQAryQ(graphStr.toCharArray(),0).second!!
  val graph1 = Array(graph0.size){i->
    IntArray(graph0[i]!!.size){j->
      graph0[i]!![j]!!
    }
  }
  var solution: Solution = Solution()
  var result  = solution.isBipartite(graph1)
