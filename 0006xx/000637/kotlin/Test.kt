...
fun main() {
  test("[3,9,20,null,15,7]",doubleArrayOf(3.0,14.5,11.0))
  test("[3,9,20,15,7]",doubleArrayOf(3.0,14.5,11.0))
}

fun test(roott: String, expected: DoubleArray) {
  println("root=%s, expected=%s".format(roott, join(expected)))
  val root = toTreeNode(roott)
  var solution: Solution = Solution()
  var result: DoubleArray = solution.averageOfLevels(root)
  println("result=%s".format(join(result)))
  aassert(result.size == expected.size)
  for(idx in result.indices){
    val diff = kotlin.math.abs(result[idx]-expected[idx])
    aassert(diff<0.00001)
  }
}
...
fun join(ary: DoubleArray): String {
  var sb: StringBuffer = StringBuffer()
  sb.append("[")
  var isFirst: Boolean=true
  for(v in ary){
