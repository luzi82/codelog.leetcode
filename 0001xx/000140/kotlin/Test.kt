...
fun main() {
  test(
    "catsanddog",
    arrayOf("cat","cats","and","sand","dog"),
    arrayOf("cats and dog","cat sand dog")
  )
  test(
    "pineapplepenapple",
    arrayOf("apple","pen","applepen","pine","pineapple"),
    arrayOf("pine apple pen apple","pineapple pen apple","pine applepen apple")
  )
  test(
    "catsandog",
    arrayOf("cats","dog","sand","and","cat"),
    arrayOf<String>()
  )
}

fun test(s: String, wordDictt: Array<String>, expectedd: Array<String>) {
  println("s=%s, wordDict=%s, expected=%s".format(s, join(wordDictt), join(expectedd)))
  val wordDict=wordDictt.toList()
  var solution: Solution = Solution()
  var result = solution.wordBreak(s,wordDict)
  var result0 = result.toTypedArray()
  println("result=%s".format(join(result0)))
  result0.sort()
  expectedd.sort()
  aassert(join(result0) == join(expectedd))
}

fun join(strAry:Array<String>):String{
