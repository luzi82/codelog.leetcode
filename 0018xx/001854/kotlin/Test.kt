...
fun main() {
  test("[[1993,1999],[2000,2010]]",1993)
  test("[[1950,1961],[1960,1971],[1970,1981]]",1960)
}

fun test(logss: String, expected: Int) {
  println("logs=%s, expected=%s".format(logss, expected))
  val logsss = toIntQAryQAryQ(logss.toCharArray(),0).second!!
  val logs = Array(logsss.size){i->
    IntArray(logsss[i]!!.size){j->
      logsss[i]!![j]!!
    }
  }
  var solution: Solution = Solution()
  var result: Int = solution.maximumPopulation(logs)
