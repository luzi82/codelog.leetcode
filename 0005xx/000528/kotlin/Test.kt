...
fun test(wStr:String) {
  val w0 = toIntQAryQ(wStr.toCharArray(),0).second!!
  val w = toIntArray(w0)
  var solution: Solution = Solution(w)

  val wSum = w.sum()
  val cntAry = IntArray(w.size)

  for(i in 0..(wSum*1000-1)){
    val ret = solution.pickIndex()
    cntAry[ret]++
  }

  println("cntAry=%s".format(join(cntAry)))

  for(i in w.indices){
    aassert(cntAry[i]>=1000*w[i]-50)
    aassert(cntAry[i]<=1000*w[i]+50)
  }
}
