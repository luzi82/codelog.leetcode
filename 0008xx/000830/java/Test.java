...
class Test {
    ...
    public static void main(String[] argv){
        ...
        test("abbxxxxzzy","[[3,6]]");
        test("abc","[]");
        test("abcdddeeeeaabbbcd","[[3,5],[6,9],[12,14]]");
        test("aba","[]");
    }
    
    public static void test(String s,String expectedStr){
        System.out.println(String.format("s=%s, expected=%s",s,expectedStr));
        Solution solution = new Solution();
        List<List<Integer>> result = solution.largeGroupPositions(s);
        int[][] intAryQAryQ = toIntAryQAryQ(result);
        System.out.println(String.format("result=%s",join(intAryQAryQ)));
        aassert(join(intAryQAryQ).equals(expectedStr));
    }

    public static int[][] toIntAryQAryQ(List<List<Integer>> intQListQListQ){
      if(intQListQListQ==null)return null;
      int[][] intAryQAry = new int[intQListQListQ.size()][];
      int i=0;
      for(List<Integer> intQListQ : intQListQListQ){
        intAryQAry[i] = toIntAryQ(intQListQ);
        ++i;
      }
      return intAryQAry;
    }
    
    public static int[] toIntAryQ(List<Integer> intQListQ){
      if(intQListQ==null)return null;
      int[] intAry = new int[intQListQ.size()];
      int i=0;
      for(int v:intQListQ){
        intAry[i] = v;
        ++i;
