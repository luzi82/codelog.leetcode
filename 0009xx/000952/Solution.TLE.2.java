...
class Solution {
    ...
    public int largestComponentSize(int[] A) {
        ...
        for(int a:A){
            ...
            int[] aFactorParentAry = new int[aFactorAry.length];
            for(int i=0;i<aFactorParentAry.length;++i){
                aFactorParentAry[i] = toParent(x,aFactorAry[i]);
            }
            int minFactorParent = min(aFactorParentAry);
            addFactorCount(x, minFactorParent,1);
            for(int aFactorParent:aFactorParentAry){
    ...
    public static int toParent(X x,int f){
