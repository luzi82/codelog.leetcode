...
fun test(boardd: String, expected: Int) {
  var boarddd = boardd
  boarddd = boarddd.replace("\n","")
  boarddd = boarddd.replace(" ","")
  println("board=%s, expected=%d".format(boarddd, expected))
  val boardIntQAryQAryQ = toIntQAryQAryQ(boarddd.toCharArray(),0).second
  val board = Array(boardIntQAryQAryQ!!.size){i->
    IntArray(boardIntQAryQAryQ!![i]!!.size){j->
      boardIntQAryQAryQ!![i]!![j]!!
    }
  }
  var solution: Solution = Solution()
  var result: Int = solution.snakesAndLadders(board)
