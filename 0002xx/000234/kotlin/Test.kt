...
fun main() {
  test(intArrayOf(1,2,2,1),true)
  test(intArrayOf(1,2),false)
}

fun test(headd: IntArray, expected: Boolean) {
  println("head=%s, expected=%s".format(join(headd), expected))
  val head = toListNode(headd)
  var solution: Solution = Solution()
  var result: Boolean = solution.isPalindrome(head)
