...
class Test {
    ...
    public static void test(String rootStr,int pInt,int qInt,int expected){
        System.out.println(String.format("root=%s, p=%d, q=%d, expected=%d",rootStr,pInt,qInt,expected));
        TreeNode root = TreeNode.toTreeNode(rootStr);
        TreeNode p=findNode(pInt,root);
        TreeNode q=findNode(qInt,root);
        Solution solution = new Solution();
        TreeNode result = solution.lowestCommonAncestor(root,p,q);
        System.out.println(String.format("result=%d",result.val));
        aassert(result.val == expected);
    }
    
    public static TreeNode findNode(int v,TreeNode node){
      if(node==null)return null;
      if(node.val==v)return node;
      TreeNode ret;
      ret = findNode(v,node.left);
      if(ret!=null)return ret;
      ret = findNode(v,node.right);
      if(ret!=null)return ret;
      return null;
    }
