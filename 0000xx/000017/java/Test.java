...
class Test {
    ...
    public static void test(String digits,String[] expectedAry){
        System.out.println(String.format("digits=%s, expected=%s",digits,join(expectedAry)));
        Solution solution = new Solution();
        List<String> resultList = solution.letterCombinations(digits);
        String[] resultAry = resultList.toArray(new String[0]);
        System.out.println(String.format("result=%s",join(resultAry)));
        Arrays.sort(resultAry);
        Arrays.sort(expectedAry);
        aassert(join(resultAry).equals(join(expectedAry)));
    }
