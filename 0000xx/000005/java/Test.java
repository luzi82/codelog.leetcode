...
class Test {
    ...
    public static void main(String[] argv){
        ...
        test("a",1);
        test("ac",1);
        // more test case from old code
        test("abb",2);
        test("banana",5);
        test("asdfdsa",7);
        test("qasdfdsappdkfahf",7);
        test("asdfccccasdf",4);
        test("asdaccccasdf",6);
        test("babadada",5);
    }
    
    public static void test(String s,int expectedLen){
        System.out.println(String.format("s=%s, expectedLen=%d",s,expectedLen));
        Solution solution = new Solution();
        String result = solution.longestPalindrome(s);
        System.out.println(String.format("result=%s",result));
        aassert(result.length() == expectedLen);
        aassert(s.contains(result));
    }
