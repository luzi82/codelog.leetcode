...
class Test {
    ...
    public static void test(String rowSumStr,String colSumStr){
        System.out.println(String.format("rowSum=%s, colSum=%s",rowSumStr,colSumStr));
        int[] rowSum = toIntAryQ(rowSumStr);
        int[] colSum = toIntAryQ(colSumStr);
        Solution solution = new Solution();
        int[][] result = solution.restoreMatrix(rowSum,colSum);
        System.out.println(String.format("result=%s",join(result)));
        aassert(result.length==rowSum.length);
        int[] resultRowSum = new int[rowSum.length];
        int[] resultColSum = new int[colSum.length];
        for(int i=0;i<rowSum.length;++i){
          aassert(result[i].length==colSum.length);
          for(int j=0;j<colSum.length;++j){
            resultRowSum[i] += result[i][j];
            resultColSum[j] += result[i][j];
          }
        }
        aassert(join(resultRowSum).equals(rowSumStr));
        aassert(join(resultColSum).equals(colSumStr));
    }
