...
class Test {
    ...
    public static void test(String rootStr){
        System.out.println(String.format("root=%s",rootStr));

        TreeNode root = TreeNode.toTreeNode(rootStr);
        Vector<Integer> oriVec = new Vector<>();
        toVec(oriVec,root);
        ...
        dfsCheck(resultVec, result);
        
        aassert(resultVec.equals(oriVec));
    }
    
    public static void toVec(Vector<Integer> intVec,TreeNode root){
      if(root==null)return;
      toVec(intVec,root.left);
      intVec.add(root.val);
      toVec(intVec,root.right);
    }
    
    public static int dfsCheck(Vector<Integer> resultVec,TreeNode result){
      if(result==null)return 0;
      int leftDepth = dfsCheck(resultVec,result.left);
      resultVec.add(result.val);
      int rightDepth = dfsCheck(resultVec,result.right);
      aassert(Math.abs(leftDepth-rightDepth)<=1);
